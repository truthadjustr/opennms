...
public class TelemetryAdapterRegistryImpl implements TelemetryAdapterRegistry {
    ...
    private static final ServiceLoader<AdapterFactory> s_serviceAdapterLoader = ServiceLoader
            .load(AdapterFactory.class);

    private static final String TYPE = "type";

    public TelemetryAdapterRegistryImpl() {
        for (AdapterFactory adapterFactory : s_serviceAdapterLoader) {
            Map<String, String> props = new HashMap<>();
            props.put(TYPE, adapterFactory.getClass().getCanonicalName());
            onBind(adapterFactory, props);
        }
    }
